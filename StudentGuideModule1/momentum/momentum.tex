...
that has both magnitude and direction. Mathematically, momentum \underline{change} 
...
with the table, using the equation above for $\Delta {\bf p}$ and the \( {{\bf p}_{i}} \) and 
\( {{\bf p}_{f}} \) from part (a). Use the same type of unit vector notation to express your 
...
(d) Calculate the change in momentum of the ball as a result of the collision, using the equation above for \( \Delta {\bf p} \) and the \( {{\bf p}_{i}} \) and \( {{\bf p}_{f}} \) from part (c).
...
is given by \( \Delta {\bf p}={\bf F}\,\Delta t\).  Hint: Begin with expression 
you wrote in part (b).
