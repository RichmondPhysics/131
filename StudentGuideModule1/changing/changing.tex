...
\textbf{Start} $\rightarrow$ \textbf{Programs} $\rightarrow$ \textbf{Physics Applications} $\rightarrow$ \textbf{131 Capstone 750} $\rightarrow$ \textbf{P, V \& A Graphs 750.cap} for all of the activities in this unit.
...
and velocity data using the ``Show coordinates and access Data Tool'' function on the graph menu bar.

(a) List 10 of the typical accelerations of the cart measured in Activity 3.
Use the ``Show coordinates etc.'' function on the acceleration \textit{vs.}~time graph to get these values. (Only
use values from the portion of the graph after the cart was released and before you stopped it.)
\answerspace{20mm}
...
Select the velocity vs. time graph and open the ``Show coordinates etc.'' function on the graph menu bar. Determine quantities \( \Delta v\) and \( \Delta t\) for two points on the velocity graph using the Delta Tool, 
