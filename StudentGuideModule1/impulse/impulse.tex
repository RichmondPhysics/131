...
by
\[
{\vec  F}=\frac{d{\vec  p}}{dt}\qquad [Eq.\: 1]\]
...
\[
\int _{t_{i}}^{t_{f}}{\vec  F}dt=\int _{t_{i}}^{t_{f}}\frac{d{\vec  p}}{dt}dt=\left( {{\vec  p}_{f}}-{{\vec  p}_{i}}\right) =\Delta {\vec  p}\qquad [Eq.\: 2]\]


Impulse is a vector quantity defined by the equation
\[
{\vec  J}=\int_{t_{i}}^{t_{f}}{\vec  F}\,dt\qquad [Eq.\: 3]\]


By combining equations {[}2{]} and {[}3{]} we can formulate the impulse-momentum
theorem in which
\[
{\vec  J}=\Delta {\vec  p}\qquad [Eq.\: 4]\]
