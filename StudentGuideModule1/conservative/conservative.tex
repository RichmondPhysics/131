...
\instructornote{%
By Will Hodge, fall 2023.  Updated to use wireless force sensor.
}
...
\textbf{Apparatus} 
\begin{itemize}[nosep]
\item \textit{Capstone} software (\filename{Force\_Sensor.cap} experiment file)
...
(a) Open the \filename{Force\_Sensor.cap} file in the \filename{\coursefolder} folder. Turn on the force sensor at your station and connect it to the computer via Bluetooth. The measurements from the force sensor may not be zero when the force is actually zero. To tare the sensor, select the desired sensor (the \textbf{Wireless Force Sensor}) in the \textbf{Controls} palette and then click \textbf{Zero Sensor Now}. 
