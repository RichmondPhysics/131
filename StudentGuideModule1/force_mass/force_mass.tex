...
(a) Open the file \filename{V\_A\_F\_Graphs.cap}  in the \filename{\coursefolder} folder. Turn on the cart at your station and connect it to the computer via Bluetooth. At the beginning of each experiment, the measurements from the built-in acceleration and forces sensors may not be zero when the acceleration or force is actually zero. To tare the sensors, select the desired sensor (either the \textbf{Smart Cart Acceleration Sensor} or the \textbf{Smart Cart Force Sensor}) in the \textbf{Controls} palette and then click \textbf{Zero Sensor Now}. When taring the sensors, the cart should be at rest and there should be no applied force on the hook.
\medskip

(b) Set up the pulley, cart, and string as shown in the figure below.
\bigskip

\answerspace{0.5cm}
%{\par\centering \includegraphics{force1/force1_fig4.eps} \par}
%{\par\centering \includegraphics{force1/cart_detector_mass.pdf} \par}
{\par\centering \includegraphics{force1/cart_pulley_mass.pdf} \par}
\answerspace{0.5cm}

(c) We will define a mass scale in which the unit is the mass of the cart, called one cart mass. Use the compact scale to determine the mass of the cart. This will be defined as one cart mass. Assemble masses which you can add to the cart to make the cart's mass equal to approximately 1.5, 2.0. 2.5 and 3.0 cart masses. Record the amount of mass to be added in each case.
\answerspace{1in}

(d) Now add masses to make the cart's mass 2.0 cart masses.

\pagebreak[3]
(e) Hang 50 g from the end of the string. Start recording data and release the car from rest.  
Stop the cart before it hits the end of the track. Repeat until you get a good run. Sketch the graphs neatly on the axes below and indicate the scale on the axes.
%\answerspace{0.3cm}
...
\end{lab_groupplot}
(f) Use the \textbf{Highlight} and \textbf{Statistics} functions to determine 
the average force and the average acceleration during the time interval when they are nearly constant, and record them below. See \textbf{Appendix \ref{capstone}} for details on the use of the \textbf{Highlight} and  \textbf{Statistics} functions of Capstone.
...
Adjust the hanging mass until the force sensor reading while the cart is accelerating
is the same as the force you recorded in the third row of the table. When
...
one meter per second per second results. %Unfortunately, the carts with force probes that we have been using typically have a mass greater than 1 kg. So we will accelerate a 2.0 kg cart with a 2.0 N force.

%(a) What do you expect the acceleration to be in this case? Show the calculation below. \vspace{20mm}

(a) Add masses to the cart so that the total mass is 1.0 kg. Set up the pulley, weighted cart, and string as in Activity 1. Hang 100 g from the end of the string. Start recording data and release the car from rest. Stop the cart before it hits the end of the track. Adjust the hanging mass, if necessary, to get an applied force of 1 N while the cart is accelerating. (Remember that when the cart is being held at rest the same hanging mass will exert more applied force on the cart than when it is accelerating!) Once you get a good run, measure the average values of force and acceleration, and record these values in a table below. Also record
...
\includegraphics{force_mass/cart_plus_two_scales.pdf}
%\vspace{0.3cm}
