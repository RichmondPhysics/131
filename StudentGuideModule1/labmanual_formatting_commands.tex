...
\usepackage{iftex}
%If compiling using pdflatex, this command makes the output file pdf 1.7, and allows up to pdf 1.7 inputs with no warning message.
\ifpdftex
   \pdfminorversion 7
\fi
...
\fancypagestyle{labfancy}[fancy]{%
	\fancyhead[LO,RE]{\slshape \rightmark} 
	\fancyhead[EL,OR]{\slshape \MakeUppercase{\headersupplementmark}\leftmark}
}
\pagestyle{labfancy}
\fancypagestyle{labcontentsfancy}[fancy]{%
	\fancyhead[LO,RE]{}
	\fancyhead[LE,RO]{\slshape \MakeUppercase{CONTENTS}}
}
...
\titleformat{\section}{\normalfont\Large\bfseries}{\headersupplementmark \thesection:}{1ex}{} % At start of section
...
\renewcommand{\part}[1]{%This replaces the code above to remove the use of \immediateaddcontentsline, which manually writes to the TOC with a \write command outside of a float or shipout context--this is prone to error in newer TeX distributions and was causing fatal compile errors 07/18/25 WH
	...
	\addcontentsline{toc}{part}{Part \Alph{tocpartnum}:\hspace{1ex}#1}
	 \phantomsection
}
...
\setlength{\cftbeforesecskip}{8.0pt plus 1.0pt minus 3.0pt} %space between chapter listings in TOC.  Default is 10.0pt plus 1.0pt
		\makeatletter
		\newcommand{\cftlotnumberline}[1]{\@cftbsnum #1\@cftasnum\@cftasnumb}
		\makeatother

		\setlength{\cftsubsecnumwidth}{1.6em}
		\setlength{\cftbeforesecskip}{7.0pt plus 1.0pt minus 4.0pt} %Default was 10.0pt plus 1.0pt
		\setlength{\cftsecnumwidth}{1.0em}
		\renewcommand{\cftmarktoc}{}
		\renewcommand{\cftsecpresnum}{\headersupplementmark}
		\renewcommand{\cftsecpresnum}{Lab~}
		\renewcommand{\cftsecaftersnumb}{:~}
		\setcounter{tocdepth}{1}
		\let\numberline\cftlotnumberline
\cftsetpnumwidth{0.3in}
\tocloftpagestyle{labcontentsfancy}
...
\newcommand{\labimport}{\clearpage\phantomsection\input}
\newcommand{\labsubimportfrom}{\clearpage\subinputfrom}
\newcommand{\includeonlylab}[1]{%
	\includeonly{#1}
	\renewcommand{\labimport}{\include}
	\renewcommand{\labsubimportfrom}{\subincludefrom}
	}
...
\phantomsection
  \IfNoValueTF  {#2}  {%NO directory {#1} {#2} {#3} \par 
	\ifnum#1=1  \labimport{#3} 
	\else \ifincludealllabs \labimport{#3} \fi 
	\fi
	}
  {%There is a directory {#1} {#2} {#3} \par
	\ifnum#1=1 \labsubimportfrom{#2} {#3} 
	\else \ifincludealllabs \labsubimportfrom{#2} {#3} \fi 
	\fi
	}
}
...
\usepackage[inline]{enumitem} %added 3/2016 by MT.  Tested 100% with other labs, looks like it does no harm.
...
%Consider re-coding this using the ``comment'' package, which allows for different versions of the text.
...
\usepackage{mfirstuc} %used for the appendix greek_alphabet

\usepackage{upgreek}
\renewcommand{\gamma}{\upgamma}
\renewcommand{\tau}{\uptau}

\usepackage{isotope} %allows formatting of isotopes, with preceding superscript and subscript for A and Z.

\newcommand{\startsquarepar}{%Command fully justifies (left and right) all lines of a paragraph, including the last line.  It's a terrible kludge to cover the fact that wrapfig doesn't play with enumitem.  Used in electronics lab on op-amps.
    \par\begingroup \parfillskip 0pt \relax}
\newcommand{\stopsquarepar}{%
    \par\endgroup}

\usepackage[lastexercise,answerdelayed]{exercise} %Used by Matt to write homework problems, separate from the labs.
...
\NewDocumentCommand \figuretoright {m O{} m +m} {%
	...
	\begin{minipage}{\textwidth - #1\textwidth - 3pt}
	#4
	\end{minipage}
	\begin{minipage}{(#1\textwidth) - 2pt}
	\medskip
...
\newcommand{\mohm}{~{\rm m}\Omega} %assumes math mode
\newcommand{\ohm}{~\Omega} %assumes math mode
\newcommand{\kohm}{~{\rm k}\Omega} %assumes math mode
\newcommand{\Mohm}{~{\rm M}\Omega} %assumes math mode

\hyphenation{im-ped-ance}


% Below, the package zref and the following line is a kluge whose only purpose is to avoid
% the compile error ``LaTeX Error: No counter 'zref@unique' defined.''
% See https://www.reddit.com/r/LaTeX/comments/u1hyvw/can_someone_decipher_this_error_for_me_latex/
\usepackage{zref}
	\makeatletter\zref@require@unique\makeatother
