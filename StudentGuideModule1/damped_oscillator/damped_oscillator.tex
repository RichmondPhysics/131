...
The vibrator should be plugged into the red and black jacks on the right side of the Pasco 550 interface, which will provide a sinusoidally varying voltage to power the vibrator.  To turn on this voltage, click the \button{Signal Generator} button on the left toolbar.  The \button{Waveform} should be set to ``Sine.''  In the \button{Frequency} box, enter ``1.00~Hz''.  You will also need to click the little curved arrow buttons just to right of that to increase the number of digits shown.  Finally, set both the \button{Amplitude} and the \button{Voltage Limit} to 8~Volts, and click the \button{On} button to start it.
